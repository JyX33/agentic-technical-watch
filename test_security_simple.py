    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            async with session.post(
                ...
                print(f"DEBUG: Valid API key test - Status: {response.status}, Response: {response_text}")
                if response.status == 200:
                    results["tests"].append({
                        "test": "valid_api_key",
                        "status": "PASS",
...
async def main():
    ...
    try:
        ...
        # Save results
        with open("/home/jyx/git/agentic-technical-watch/simple_security_test_report.json", "w") as f:
